...
#上下文应用处理器--时间,将online_time变成全局变量,admin中可以直接调用online_time
...
#登陆装饰器
def admin_login_req(f):     #定义装饰器，验证登陆时，session中用户是否已验证通过
	@wraps(f)
...
def login():
	...
	if form.validate_on_submit():     #表单提交验证
		...
		return redirect(request.args.get('next') or url_for('admin.index'))  #request.args.get('next')代表之前访问的页面
	return render_template('admin/login.html', form=form)

@admin.route('/logout/')
@admin_login_req
def logout():
	session.pop('admin', None)    #登陆时保存到session的用户，去除
...
def role_add():
	...
	if form.validate_on_submit():
		...
		if name != 0:
			...
			role = Role(
				name = data['name'],
				auths =','.join(map(lambda v:str(v),data['auths'])),  #将数据变成字符串
				)
			db.session.add(role)
			db.session.commit()
			flash('添加角色成功','ok')
...
def role_edit(id=None):
	...
	if request.method=='GET':
		...
		form.auths.data = list(map(lambda v:int(v), auths.split(','))) #数据库转化为字典
	if form.validate_on_submit():
		data = form.data
